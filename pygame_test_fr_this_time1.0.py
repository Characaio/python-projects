...
import math
pg.init()
#tecla P mostra as particulas
#tecla Q ativa as particulas com gravidade
#tecla E ativa as particulas de gas
#tecla Z spawna as particulas
#tecla C ativa um atrator gravitacional ao seu mouse
#tecla V fixa o atrator gravitacional 
#tecla R limpa as particulas da tela
#tecla B faz um repel ao redor do mouse
BLACK = (0,0,0)
WHITE = (255, 255, 255)
    ...
    def update(self):
        if fric: #ativa a fricção
            ...
            if self.momentu < terminal:
                self.momentu += self.momentum
            self.vy -= -float(self.gravity/50)
            # self.vy - float(self.gravity/100) * self.momentu
        elif gas: #ativa a fisica de gas
        ...
        # define particulas saltitantes, mas não funciona
            ...
            self.pos[0] += self.vx * self.velocity
            self.pos[1] -= self.gravity * self.momentu
            self.momentu += self.momentum
        ...
        #mas, desativa a colisão
            ...
            if rdist < self.repels+self.size and repel:
                ...
                self.pos[1] += self.vy * self.momentu 
                self.vx *= 0.999 #quanto menor esses dois valores forem,
                self.vy *= 0.999 #maior sera a perda de velocidade
                if self.momentu > terminal:
                    self.momentu -= 2
                if self.momentu < terminal:
                    self.momentu += self.momentum
            ...
            if not attract or not gravidade or not gas or not bouncy:
                self.pos[0] += self.vx * self.velocity
...
while run:
    ...
    for i,p1 in enumerate(particles):
        for j in range(i+1, len(particles)): #isso faz essas duas funções para todas particulas presentes
            if not attract or pause:
                #não colide se estiver pausado ou com orbita
